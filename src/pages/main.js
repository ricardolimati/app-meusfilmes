...
export default class Main extends Component {
  ...
  loadFilmes = async (page = 1) => {
    ...
    const response = await filmes.get(`/${page}?sort=year&order=-1&genre=all`);
  ...
  renderItem = ({ item }) => (
    ...
    <View style={styles.filmContainer}>
      ...
      <View style={styles.filmDados}>
        ...
        <Text style={styles.filmGenero}>{item.genres.map(function(gen){ return gen+" / "})}</Text>
        <TouchableOpacity 
            style={styles.filmButton}
            onPress={() => {
              this.props.navigation.navigate('Filmes', { filme: item });
            }}
            >
